pub mod test_cross_entry_state {
    ...
    impl<H: tree_hash::TreeHashDigest> tree_hash::TreeHash<H> for State {
        ...
        fn tree_hash_root(&self) -> H::Output {
            ...
            hasher
                .write(
                    <_ as tree_hash::TreeHash<H>>::tree_hash_root(&self.data).as_ref(),
                )
                .expect("tree hash derive should not apply too many leaves");
            hasher
                .write(
                    <_ as tree_hash::TreeHash<H>>::tree_hash_root(&self.counter).as_ref(),
                )
                .expect("tree hash derive should not apply too many leaves");
...
pub mod test_cross_entry_update {
    ...
    impl<H: tree_hash::TreeHashDigest> tree_hash::TreeHash<H> for Update {
        ...
        fn tree_hash_root(&self) -> H::Output {
            ...
            hasher
                .write(
                    <_ as tree_hash::TreeHash<H>>::tree_hash_root(&self.state).as_ref(),
                )
                .expect("tree hash derive should not apply too many leaves");
            hasher
                .write(
                    <_ as tree_hash::TreeHash<H>>::tree_hash_root(&self.timestamp)
                        .as_ref(),
                )
                .expect("tree hash derive should not apply too many leaves");
            hasher
                .write(
                    <_ as tree_hash::TreeHash<H>>::tree_hash_root(&self.updates).as_ref(),
                )
                .expect("tree hash derive should not apply too many leaves");
            hasher.finish().expect("tree hash derive should not have a remaining buffer")
        }
