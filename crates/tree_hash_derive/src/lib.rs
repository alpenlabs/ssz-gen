    ...
    let output = quote! {
        impl #impl_generics tree_hash::TreeHash for #name #ty_generics #where_clause {
            ...
            fn tree_hash_root<__H: tree_hash::TreeHashDigest>(&self) -> __H::Output {
                let mut hasher = tree_hash::MerkleHasher::<__H>::with_leaves(#num_leaves);

                #(
                    hasher.write(<_ as tree_hash::TreeHash>::tree_hash_root::<__H>(&self.#idents).as_ref())
                        .expect("tree hash derive should not apply too many leaves");
                )*

                let result = hasher.finish().expect("tree hash derive should not have a remaining buffer");
                result
            }
        }
    };
    ...
    let output = quote! {
        impl #impl_generics tree_hash::TreeHash for #name #ty_generics #where_clause {
            ...
            fn tree_hash_root<__H: tree_hash::TreeHashDigest>(&self) -> __H::Output {
                ...
                let mut field_roots: Vec<<__H as tree_hash::TreeHashDigest>::Output> =
                    Vec::with_capacity(#num_fields);
                #(
                    if self.#idents.is_some() {
                        field_roots.push(<_ as tree_hash::TreeHash>::tree_hash_root::<__H>(&self.#idents));
                    }
                )*
                let hash = tree_hash::merkleize_progressive_with_hasher::<__H>(&field_roots);
                let active_fields_hash = <_ as tree_hash::TreeHash>::tree_hash_root::<__H>(&active_fields);

                <__H as tree_hash::TreeHashDigest>::hash32_concat(hash.as_ref(), active_fields_hash.as_ref())
            }
        }
    };
    output.into()
    ...
    for (ty, ident, field_opt) in parse_tree_hash_fields(struct_data) {
        ...
        if is_optional {
            ...
            field_root_pushes.push(quote! {
                if self.#ident.is_some() {
                    field_roots.push(<_ as tree_hash::TreeHash>::tree_hash_root::<__H>(&self.#ident));
                }
            });
        } else {
            set_active_fields.push(quote! {
    ...
    let output = quote! {
        impl #impl_generics tree_hash::TreeHash for #name #ty_generics #where_clause {
            ...
            fn tree_hash_root<__H: tree_hash::TreeHashDigest>(&self) -> __H::Output {
                match self {
                    #(
                        #patterns => <_ as tree_hash::TreeHash>::tree_hash_root::<__H>(inner),
                    )*
                }
            }
        }
    ...
    let patterns: Vec<_> = enum_data
        ...
        .map(|(i, variant)| {
            ...
            if fields_len == 0 {
                quote! {
                    #name::#variant_name => {
                        let root = <__H as tree_hash::TreeHashDigest>::get_zero_hash(0);
                        let selector = #selector_index;
                        tree_hash::mix_in_selector_with_hasher::<__H>(&root, selector)
                ...
                quote! {
                    #name::#variant_name(inner) => {
                        let root = <_ as tree_hash::TreeHash>::tree_hash_root::<__H>(inner);
                        let selector = #selector_index;
                        tree_hash::mix_in_selector_with_hasher::<__H>(&root, selector)
                            .expect("derive macro should prevent out-of-bounds selectors")
                    }
